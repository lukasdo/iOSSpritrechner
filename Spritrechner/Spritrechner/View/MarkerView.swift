...
protocol ShowRouteDelegate {
    func showRouteDelegate(annotation: MKAnnotation)
}

class MarkerView: MKMarkerAnnotationView {
    
    var routeDelegate: ShowRouteDelegate?
    
    override var annotation: MKAnnotation? {
      willSet {
        // 1 Passing argumetns to selector: https://stackoverflow.com/questions/43251708/passing-arguments-to-selector-in-swift
        guard (newValue as? Tanke) != nil else { return }
        canShowCallout = true
        calloutOffset = CGPoint(x: -5, y: 5)
        let button = UIButton(type: .detailDisclosure)
//        button.addTarget(self, action: #selector(self.showRouteAnno), for: UIControl.Event.touchUpInside)
...
//    private let contentInsets = UIEdgeInsets(top: 10, left: 10, bottom: 10, right: 40)
//
...
//        let stackView = UIStackView(arrangedSubviews: [labelVibrancyView])
